...
export default function Cars() {
    ...
    useEffect(() => {
        ...
        fetch(`http://127.0.0.1:8000/api/vehicles`)
            ...
            .then((res) => {
                if (res?.data) {
                    data = res.data.map((element) => ({
                        ...element,
                        photoURL: `http://127.0.0.1:8000${element?.photo}`,
                    }));
                    setVehicles(data);
                }
            })
            .catch((error) => {
                console.error(
                    "There was an error fetching the vehicles data!",
                    error
                );
            });
    }, []);
    ...
    return (
        <div>
            <header
                className=" relative w-full h-[70vh] bg-cover bg-center flex flex-col justify-center items-center gap-4"
                style={{
                    backgroundImage:
                        'url("https://duruthemes.com/demo/html/renax/dark/img/slider/3.jpg")',
                }}
            >
                <div className=" inset-0 bg-black opacity-30 w-full h-full z-10  absolute" />
                <p className="text-[#f5B754] font-thin z-20">SELECT YOUR CAR</p>
                <h1 className="text-3xl md:text-5xl z-20">
                    Luxury{" "}
                    <span className="text-[#f5B754] font-thin">Car Fleet</span>
                </h1>
            </header>
            <main className="flex flex-col gap-x-10  gap-y-40 lg:flex-row flex-wrap justify-center my-12 items-center">
                {vehicles?.map((vehicle) => {
                    return (
                        <div
                            key={vehicle?.id}
                            className="relative w-[90%] md:w-[70%] lg:w-[40%] lg:flex lg:flex-col justify-end items-center "
                        >
                            <img
                                className=" rounded-[20px] "
                                src={vehicle?.photoURL}
                            />
                            <ul className=" absolute lg:bottom-[-80px] w-full lg:w-[97%] flex flex-row flex-wrap md:flex-nowrap items-center justify-center sm:justify-between p-1  sm:p-3 bg-[#222222]  rounded-3xl">
                                <li>
                                    <h3 className="text-xl sm:text-1xl my-1 sm:my-2  capitalize">
                                        {vehicle?.make} {vehicle?.model}
                                    </h3>
                                    <ul className="flex flex-row flex-wrap items-center gap-5">
                                        <li className="flex flex-col text-[#9b9b9b] text-sm items-center">
                                            <svg
                                        ...
                                        <li className="flex flex-col text-[#9b9b9b] text-sm items-center">
                                            <Luggage className="text-[#f5B754] w-5" />
                                            <span>2 Bags</span>
                                        </li>
                                    </ul>
                                </li>
                                <li className="flex flex-row text-sm items-center">
                                    <Link to={`/cars/${vehicle?.id}`}>
                                        <Button
                                            size="lg"
                                            className="bg-[#f5B754] min-w-[110px] my-2 hover:bg-white text-black rounded-full "
                                        >
                                            Details
                                        </Button>
                                    </Link>
                                    <p className="m-2">
                                        <span className="text-lg text-[#f5B754]">
                                            $750
                                        </span>
                                        <br />
